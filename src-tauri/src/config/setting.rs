...
fn read_setting(path: &Path) -> Option<Setting> {
...
fn write_setting(path: &Path, setting: &Setting) {
...
fn migrate_legacy_setting<R: Runtime>(app_handle: &AppHandle<R>, path: &Path) -> Option<Setting> {
...
pub fn get_store_dat_setting<R: Runtime>(app_handle: &AppHandle<R>) -> Setting {
    ...
    let setting = read_setting(&path)
        ...
        .unwrap_or_default();
    let should_persist_normalized = setting.dsh_launch_options_version == 0;
    let setting = normalize_setting(setting);
    if should_persist_normalized {
        write_setting(&path, &setting);
    }
    setting
}
...
pub fn validate_dsh_environment(environment: &[DshEnvironmentVariable]) -> Result<(), String> {
...
/// 记录已安装 Harness 发行版对应的 GitHub release commit hash。
...
/// 记录已安装 Harness 发行版对应的 GitHub release tag。
...
mod tests {
    ...
    #[test]
    fn reads_install_root_store_with_current_metadata() {
        ...
        let expected = Setting {
            ...
            dsh_web_port: "3913".to_string(),
            dsh_profile: "web".to_string(),
            dsh_host: "127.0.0.1".to_string(),
            dsh_pkg_commit: Some("commit".to_string()),
            dsh_pkg_tag: Some("tag".to_string()),
        ...
        assert_eq!(actual.dsh_pkg_tag, expected.dsh_pkg_tag);
        assert_eq!(actual.http_proxy, expected.http_proxy);
        assert!(actual.preinstall_done);
        fs::remove_file(path).unwrap();
    }

    #[test]
    fn restores_launch_options_from_legacy_setting() {
    ...
    fn rejects_reserved_launch_options() {
        assert!(validate_dsh_environment(&[DshEnvironmentVariable {
