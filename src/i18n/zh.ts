import { Translations } from "./types";

export const zh: Translations = {
  app: {
    title: "DeepSeek Harness 桌面版",
    retry: "重试",
    ready: "DeepSeek Harness 已就绪！",
    open_editor: "打开 DeepSeek Harness",
    expand_sidebar: "展开侧边栏",
    collapse_sidebar: "收起侧边栏",
    restart: "重启",
    shutdown: "停止",
    open_browser: "在浏览器中打开",
    copy_url: "复制地址",
    reveal_dir: "打开数据目录",
    refresh: "刷新",
  },
  status: {
    checking: "正在检查系统环境...",
    installing: "正在安装依赖...",
    updating: "正在更新 DeepSeek Harness...",
    preparing_engine: "正在准备 Node.js 运行时... {{progress}}%",
    downloading_harness: "正在下载 DeepSeek Harness... {{progress}}%",
    extracting: "正在解压资源包...",
    starting: "正在启动 DeepSeek Harness 服务...",
    ready: "服务已就绪",
    error: "启动失败",
    loading: "正在加载界面...",
  },
  update: {
    available: "发现新版本 {{tag}}，是否立即更新？",
    now: "立即更新",
    later: "稍后",
  },
  errors: {
    service_start_timeout: "DeepSeek Harness 启动超时，请检查端口 {{port}} 是否被占用或启动过慢。",
    install_verify_failed: "资源包已下载，但安装校验失败。",
    network_timeout: "网络下载超时，请检查网络连接后重试。",
    unknown: "未知错误",
  },
  ui: {
    service_url: "服务地址",
    port: "端口",
    auto_start: "启动时自动运行",
    http_proxy: "HTTP 代理",
    http_proxy_placeholder: "http://127.0.0.1:7890（留空直连）",
    harness_launch: "Harness 启动选项",
    dsh_environment: "环境变量",
    dsh_arguments: "命令行参数",
    name: "名称",
    value: "值",
    app_info: "应用信息",
    current_version: "当前版本",
    dsh_version: "Harness 版本",
    node_version: "Node.js 运行时",
    data_dir: "数据目录",
    logs: "服务日志",
    no_logs: "（暂无日志）",
    save: "保存",
    saved: "已保存",
    loading_interface: "正在加载 DeepSeek Harness 界面...",
    iframe_error: "无法加载 DeepSeek Harness 界面。",
    ensure_running: "请确认服务正在 {{url}} 运行。",
    language: "语言",
    actions: "操作",
    connection_status: "连接状态",
    running: "运行中",
    stopped: "已停止",
    settings: "设置",
    minimize: "最小化",
    maximize: "最大化",
    restore: "还原",
    close: "关闭",
    drag_window: "拖动窗口，双击最大化",
    zoom_level: "界面缩放：{{level}}%",
    zoom_minimum: "已达到最小缩放比例",
    zoom_maximum: "已达到当前窗口的最大缩放比例",
    waiting_logs: "等待安装日志...",
    install_log: "安装日志",
  },
  buttons: {
    retry: "重试",
    save: "保存",
    copy: "复制",
    clear_logs: "清空日志",
    refresh_logs: "刷新",
    add: "新增",
    remove: "删除",
  },
  messages: {
    copy_success: "地址已复制到剪贴板",
    copy_failed: "复制地址失败",
    config_saved: "配置已保存",
    save_failed: "保存配置失败",
    start_failed: "启动服务失败",
    stop_failed: "停止服务失败",
    restarting: "正在重启服务...",
    logs_cleared: "日志已清空",
  },
};
